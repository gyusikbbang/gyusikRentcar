<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%@ page import = "rent.cus.rentcDBBean" %>
<%@ page import = "rent.cus.rentcDataBean" %>
...
<%
    ...
    rentcDBBean dbPro = rentcDBBean.getInstance();
...
.menu li:nth-child(6) a::before{ content: '\e8b8'; }
	...
	<body>
		...
		<table>
			...
			<button type="button" class="button1" onclick="location.href='Rent_customer_search.jsp'">고객 검색</button>
			<caption><br></caption>
			<caption><a style="float:left; font-size: 1.5em;" >고객 정보(총 <%=count%>)</a></caption>
			<caption><br></caption>
			<thead>
						...
						<% if (count == 0) { %>
							...
							<%  
							   for (int i = 0 ; i < articleList.size() ; i++) {
								   rentcDataBean article = articleList.get(i);
							%>
							<td  width="50" > <%=article.getCusid()%></td>
							<td><%=article.getCusName()%></td>
							<td><%=article.getCusAddr()%></td>
							<td><%=article.getCusEmail()%></td>
							<td><%=article.getUseNum()%></td>
							<td><%=article.getUct()%></td>
							<td><%=article.getUseDate()%></td>
							...
							<div class="text-center">
								...
								%>
										<li class="page-item">
										<a class="page-link<% if(startPage < 10) out.print(" disabled"); %>" href="Rent_customer_Select.jsp?pageNum=<%= startPage - 10 %>" aria-label="Previous">
											<span aria-hidden="true">&laquo;</span>
										</a>
										</li>
								<% 
									for (int i = startPage; i <= endPage; i++) { %>
										<li class="page-item<% if(i == currentPage) out.print(" active"); %>"><a class="page-link" href="Rent_customer_Select.jsp?pageNum=<%= i %>"><%= i %></a></li>
								<%  } %>
										<li class="page-item">
											<a class="page-link<% if(endPage >= pageCount) out.print(" disabled"); %>" href="Rent_customer_Select.jsp?pageNum=<%= startPage + 10 %>" aria-label="Next">
					...
					<div class="menu">
						...
						<ul>
							<li><a href="Rent_customer_Form.jsp" class="item"><div>추가</div></a></li>
							<li><a href="Rent_customer_update_Form.jsp" class="item"><div>수정</div></a></li>
							<li><a href="Rent_customer_delete_Form.jsp" class="item"><div>삭제</div></a></li>
						</ul>
					</div>
