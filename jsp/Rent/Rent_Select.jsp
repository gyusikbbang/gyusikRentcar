<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%@ page import = "rent.r.rentDBBean" %>
<%@ page import = "rent.r.rentDataBean" %>
...
<%
    ...
    rentDBBean dbPro = rentDBBean.getInstance();
...
.menu li:nth-child(6) a::before{ content: '\e8b8'; }
	...
	<body>
		...
		<table>
			<button type="button" class="button1" onclick="location.href='Rent_search.jsp'">대여ID 검색</button>
			<button type="button" class="button2" onclick="location.href='Rent_price_sum.jsp'">고객 청구금액 검색</button>
			<caption><br></caption>
			<caption><a style="float:left; font-size: 1.5em;" >대여 정보(총 <%=count%>)</a></caption>
			<caption><br></caption>
			<thead>
						...
						<% if (count == 0) { %>
							...
							<%  
							   for (int i = 0 ; i < articleList.size() ; i++) {
								   rentDataBean article = articleList.get(i);
							%>
							
							   <tr height="30">
								<td  width="50" > <%=article.getCarRentid()%></td>
								<td><%=article.getCarId()%></td>
								<td><%=article.getCusDriverLicenseId()%></td>
								<td><%=article.getRentStrartDate()%></td>
								<td><%=article.getRentEndDate()%></td>
								<td><%=article.getRentPeriod()%></td>
								<td><%=article.getPrice()%></td>
								<td><%=article.getPriceDeadline()%></td>
								<td><%=article.getAddPrice()%></td>
								<td><%=article.getAddContents()%></td> 
							  </tr>
							<%}%>
							</table>
							<%}%>
...
<tfoot>
	...
	<div class="menu">
		...
		<ul>
			<li><a href="Rent_Form.jsp" class="item"><div>추가</div></a></li>
			<li><a href="Rent_update_Form.jsp" class="item"><div>수정</div></a></li>
			<li><a href="Rent_delete_Form.jsp" class="item"><div>삭제</div></a></li>
		</ul>
	</div>		
</html>
<% 
