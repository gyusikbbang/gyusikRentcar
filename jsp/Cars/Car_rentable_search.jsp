...
<div class="animated bounceInDown">
  <div class="container">
	...
	<table>
		<caption><a style="float:left; font-size: 1.5em;" >대여가능 캠핑카ID 검색</a></caption>
		<caption><br></caption>
		<thead>
			<tr>
				<th>이미지</th>
	...
	<tbody>
		<%
			...
			conn = DriverManager.getConnection(url, user, password);
				...
				String sql= "select distinct c.img, c.car_id , c.com_id, c.car_name, c.car_type, c.car_number, c.reg_date, c.nop, c.car_info ,c.car_rent_price ,c.rent_num ,  c.car_repair  from cars c, rent r where ((c.car_id = r.car_id AND TO_CHAR(r.rent_end_date, 'YYYYMMDD') < TO_CHAR(SYSDATE, 'YYYYMMDD') AND c.car_repair = 'X') OR ( c.rent_num = '0' AND car_repair ='X')) ";
				
				
				if(!"".equals(word)){
					word = "%" + word + "%";
					sql += " and c.car_id like ? " ;					
				}
				...
				while(rs.next()){
							...
							String car_info =rs.getString("car_info");
							String car_rent_price =rs.getString("car_rent_price");
							String rent_num =rs.getString("rent_num");
							String car_repair =rs.getString("car_repair");
		...
		<tr>
			<td><img src="/book/imageFile/<%=img%>"></td>
			<td><%=car_id%></td>
								<td><%=com_id%></td>
								<td><%=car_name%></td>
								<td><%=car_type%></td>
...
<tfoot>
	<td colspan="10" class="tablefoot"></td>
	   ...
	   <br>
	   <br>
      <span><a href="Cars_Select.jsp">돌아가기</a></span>
    </div>
</div>
