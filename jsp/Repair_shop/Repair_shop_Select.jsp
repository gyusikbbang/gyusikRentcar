<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%@ page import = "rent.shop.repairshopDBBean" %>
<%@ page import = "rent.shop.repairshopDataBean" %>
...
<%
    ...
    repairshopDBBean dbPro = repairshopDBBean.getInstance();
...
.menu li:nth-child(6) a::before{ content: '\e8b8'; }
	...
	<body>
			...
			<ul>
				...
				<div style="float:right;" role="group" aria-label="Basic radio toggle button group">
					<span style="color: #ffffff;"><%=id %></span>
						<form method="post" action="/book/member/sessionLogout.jsp">
		...
		<table>
			...
			<button type="button" class="button1" onclick="location.href='Repair_shop_search.jsp'">정비소 검색</button>
			<caption><br></caption>
			<caption><a style="float:left; font-size: 1.5em;" >정비소(총 <%=count%>)</a></caption>
			<caption><br></caption>
			<thead>
						...
						<% if (count == 0) { %>
							...
							<%  
							   for (int i = 0 ; i < articleList.size() ; i++) {
								   repairshopDataBean article = articleList.get(i);
							%>
							   <tr height="30">
								<td  width="50" > <%=article.getReShid()%></td>
							<td><%=article.getShopName()%></td>
							
							<td><%=article.getShopAddr()%></td>
							<td><%=article.getShoptel()%></td>
							<td><%=article.getShopM()%></td>
							<td><%=article.getShopMemail()%></td>
							...
							<div class="text-center">
								...
								%>
										<li class="page-item">
										<a class="page-link<% if(startPage < 10) out.print(" disabled"); %>" href="Repair_shop_Select.jsp?pageNum=<%= startPage - 10 %>" aria-label="Previous">
											<span aria-hidden="true">&laquo;</span>
										</a>
										</li>
								<% 
									for (int i = startPage; i <= endPage; i++) { %>
										<li class="page-item<% if(i == currentPage) out.print(" active"); %>"><a class="page-link" href="Repair_shop_Select.jsp?pageNum=<%= i %>"><%= i %></a></li>
								<%  } %>
										<li class="page-item">
											<a class="page-link<% if(endPage >= pageCount) out.print(" disabled"); %>" href="Repair_shop_Select.jsp?pageNum=<%= startPage + 10 %>" aria-label="Next">
					...
					<div class="menu">
						...
						<ul>
							<li><a href="Repair_shop_Form.jsp" class="item"><div>추가</div></a></li>
							<li><a href="Repair_shop_update_Form.jsp" class="item"><div>수정</div></a></li>
							<li><a href="Repair_shop_delete_Form.jsp" class="item"><div>삭제</div></a></li>
						</ul>
					</div>
